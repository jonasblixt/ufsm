...
sq_template = """
static unsigned int sq_len(struct {model_name}_machine *m)
{{
    if (m->head == m->tail)
        return 0;
    if (m->tail < m->head)
...
static unsigned int sq_push(struct {model_name}_machine *m, unsigned int signal)
{{
...
static unsigned int sq_pop(struct {model_name}_machine *m)
{{
...
def _gen_c_head(hmodel, fmodel, sq_length, f):
    ...
    if len(hmodel.signals) > 0:
        _emit(f, 0, sq_template.format(queue_len=sq_length, model_name=hmodel.name))
...
def _gen_machine_struct(hmodel, sq_length, f):
    ...
    _emit(f, 1, f"unsigned int signal[{sq_length}];")
    ...
    for ex in ft.exits:
        ...
        for a in ex.actions:
            if isinstance(a, ActionFunction):
                ...
                _emit(
                    f, indent + indent_extra, f"if(sq_push(m, {a.signal.index}) != 0)"
                )
...
def _gen_reset_vector(hmodel, fmodel, f):
    ...
    for a in fmodel.isv.actions:
        if isinstance(a, ActionFunction):
            ...
            _emit(f, 3, f"if(sq_push(m, {a.signal.index}) != 0)")
            _emit(f, 4, f"return -UFSM_SIGNAL_QUEUE_FULL;")
